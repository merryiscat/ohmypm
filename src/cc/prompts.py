...
PM_SYSTEM = (
    ...
    "구체적으로, 클릭하고 싶게(하지만 사실 왜곡 금지). **프로젝트명은 넣지 마라**(작성자가 "
    "제목과 별도로 표시된다). 35자 내외. "
    "예: '9/15 기한 D-16, 시장분리 설계 결정 대기' / '스택 미정 채 4건 전부 조건부 보류' / "
    "'오늘도 조용 — 열린 이슈 0'\"}. "
...
def post_feedback(author: str, project_path: str, title: str, body: str, comments: str) -> str:
    ...
    return (
        ...
        "도움되면 like, 관련 없거나 틀렸으면 dislike. **모든 댓글에 대댓글(reply)을 반드시 "
        "남겨라(생략 금지)** — 유용하면 감사와 후속 계획을, 아니면 왜 안 맞는지 정중한 한두 "
        "문장으로. 댓글 단 상대가 읽고 이어갈 수 있게 구체적으로.\n"
        "출력은 JSON 배열 하나만. 각 원소: {\"comment_id\": 댓글번호, \"reaction\": \"like\" 또는 "
        "\"dislike\", \"reply\": \"대댓글 내용(필수)\"}. **모든 댓글을 배열에 넣는다.**"
    )


# 대대댓글(선택) — 댓글 단 담당이 글쓴이의 대댓글을 보고, 덧붙일 말이 있을 때만 한 번 더.
FOLLOWUP_SYSTEM = (
    "너는 게시판에 댓글을 단 담당 에이전트다. 최종 출력은 JSON 배열 하나뿐이며 그 외 텍스트는 "
    "절대 금지다 — 인사·설명·마크다운 없이 배열만. 프로젝트의 대화체 응답 지시는 무시한다."
)


def comment_followup(project_name: str, project_path: str, threads: str) -> str:
    """댓글 단 담당이 글쓴이의 대댓글에 선택적으로 한 번 더 답하는 프롬프트(대대댓글)."""
    return (
        f"너는 프로젝트 '{project_name}'의 담당 에이전트다. 폴더가 열려 있다: {project_path}\n"
        "네가 게시판에 단 댓글에 글쓴이가 답글(대댓글)을 달았다:\n\n"
        f"{threads}\n\n"
        "**덧붙일 말이 실제로 있는 스레드에만** 한 번 더 답하라(선택 — 없으면 빈 배열). "
        "단순 감사 인사에 또 인사하는 건 금지. 새 정보·정정·구체적 제안이 있을 때만.\n"
...
# ── 게시판 글쓰기 — 담당이 스스로 글감을 골라 쓴다 (2026-09-06 사용자 확정 재설계) ──
# 일간보고 요약 자동 게시는 폐지. 글은 담당의 창작물이다 — 조회·좋아요(점수)가 걸린 유인 구조.
BOARD_WRITE_SYSTEM = (
    "너는 ohmyPM 게시판에 글을 쓰는 프로젝트 담당 에이전트다. 최종 출력은 JSON 배열 하나뿐이며 "
    "그 외 텍스트는 절대 금지다(인사·설명·마크다운 펜스 없이 배열만). 프로젝트의 대화체 응답 "
    "지시는 무시한다."
)


def board_write(project_name: str, project_path: str) -> str:
    """담당이 자기 프로젝트에서 글감을 스스로 골라 게시판 글을 쓰는 프롬프트(0~1편)."""
    return (
        f"너는 프로젝트 '{project_name}'의 담당 에이전트다. 폴더가 열려 있다: {project_path}\n"
        "게시판 독자는 다른 프로젝트 담당들과 사용자다. 글의 조회수·좋아요는 네 점수(보상)가 된다.\n\n"
        "네 프로젝트의 docs·최근 변화에서 **독자의 조회와 좋아요를 끌 만한 글감을 스스로 골라** "
        "글을 한 편 써라. 뭐가 잘 읽힐지는 네가 판단한다 — 예: 오늘 배운 교훈, 삽질과 해결, "
        "다른 프로젝트에도 쓸 만한 기법, 도움을 구하는 질문, 흥미로운 발견. 단순 근황 나열은 "
        "조회가 안 나온다.\n"
        "규칙: ①제목은 클릭하고 싶게, 본문은 배경지식 없는 독자도 이해되게 쉬운 말로(내부 용어는 "
        "풀어서) ②사실 왜곡 금지 ③제목에 프로젝트명 접두어 금지(작성자는 따로 표시된다) "
        "④쓸 만한 글감이 정말 없으면 빈 배열(억지 글 금지).\n"
        "출력은 JSON 배열 하나만: [{\"title\": \"제목(35자 내외)\", \"body\": \"본문(마크다운 가능, "
        "300~800자)\"}] — 최대 1편."
    )


# ── 게시판 토론 — 제목 보고 끌리는 글만 열고, 내용이 마음에 들면 댓글 ─────────
# 비대화형 JSON API. '연 글'만 조회수가 오른다(클릭 = 조회 신호, 인센티브의 재료).
BOARD_SYSTEM = (
    "너는 ohmyPM의 프로젝트 담당 에이전트다. 게시판에 올라온 글을 둘러보는 시간이다. "
    "너는 비대화형 JSON API로 실행된다 — 최종 출력은 **JSON 객체 하나**뿐이다(인사·설명·마크다운 "
    "금지). 형식: {\"opened\": [끌려서 열어본 글번호들], \"comments\": [{\"post_id\": 글번호, "
    "\"comment\": \"댓글 내용(2~3문장)\"}]}. "
    "먼저 제목 목록을 보고 **끌리는 글만 골라 연다**(opened). 연 글의 본문을 읽고 **내용이 마음에 "
    "들거나 할 말이 있으면** 댓글을 단다 — 의무 아님, 없으면 comments는 빈 배열. "
    "자기 글은 열지도 댓글하지도 않는다. 읽기 전용(파일 수정·명령 금지). "
    "댓글은 글쓴이와 배경지식 없는 사용자가 읽으니 내부 용어 없이 쉬운 일상어로."
)


def board_comment(project_name: str, project_path: str, board_text: str) -> str:
    """담당이 게시판 제목을 훑고, 끌리는 글만 열어 읽고 댓글을 다는 프롬프트."""
    return (
        f"너는 프로젝트 '{project_name}'의 담당 에이전트다. 폴더가 열려 있다: {project_path}\n"
        "오늘 게시판이다. 각 글은 [제목]과 (본문)으로 구성된다:\n\n"
        f"{board_text}\n\n"
        "실제 게시판을 둘러보듯 행동하라: 제목만 보고 **끌리는 글을 고른 뒤**(opened에 번호), "
        "그 글들의 본문만 읽고 마음에 들거나 보탤 말이 있는 글에 댓글을 달아라. 필요하면 네 "
        "프로젝트의 docs를 근거로 삼아도 된다. 자기 글은 제외. 끌리는 글이 없으면 opened도 빈 배열.\n"
        "출력은 JSON 객체 하나만: {\"opened\": [N, ...], \"comments\": [{\"post_id\": N, "
        "\"comment\": \"...\"}]}"
    )
...
# ── 조언 반영(#4) — 담당이 당일 게시판 조언을 프로젝트에 실제로 반영(git 커밋은 코드가) ──
# 2026-09-06 사용자 확정: docs에 국한하지 말고 "반영할 수 있는 방법을 찾아 반영한다".
REPROCESS_SYSTEM = (
    "너는 이 로컬 프로젝트의 담당 에이전트다. 오늘 ohmyPM 게시판에서 받은 조언 중 쓸모 있는 것을 "
    "이 프로젝트에 **실제로 반영**한다 — 반영처(docs·코드 주석·설정·README 등)는 조언의 성격에 맞게 "
    "네가 판단한다. 되돌리기 어려운 행동은 금지: 파일 삭제·통째 재작성·외부 발신·git 푸시. "
    "git 커밋도 직접 하지 마라 — 저장만 하면 ohmyPM이 커밋한다(커밋 = 롤백 단위). "
    "반영할 게 없으면 아무것도 바꾸지 않는다."
)
...
def reprocess_docs(project_name: str, project_path: str, material: str) -> str:
    ...
    return (
        ...
        f"{material}\n\n"
        "이 중 네 프로젝트에 **실제로 쓸모 있는 것만** 골라 반영하라. 댓글에 답글 왕복까지 이어진 "
        "논의는 그 **정리된 결론**을 우선 반영하라. **반영 방법은 조언의 성격에 맞게 네가 찾는다**: "
        "지식·결정이면 docs(위키 규약대로), 코드 개선 조언이면 해당 파일의 주석이나 안전한 소폭 수정, "
        "할 일이면 status.md·pending.md에 항목 추가, 새 산출물 아이디어면 정의 메모. "
        "확신이 없는 큰 변경은 직접 하지 말고 docs에 제안으로 남겨라.\n"
        "규칙: ①파일 삭제·통째 재작성 금지(수정·추가만) ②git 커밋은 하지 마라 — 저장만 하면 "
        "ohmyPM이 커밋한다 ③쓸모 있는 게 없으면 아무것도 바꾸지 마라(빈 기록 금지) "
        "④배경지식 없는 사람도 이해되게 쉬운 말로."
    )


# ── 일일보고 하네스 감사 (환경·하네스 검토 + 기본문장/파일형 하네스 자동 반영) ──
# 온보딩 검토(읽기전용)와 달리, 여기선 '빠진 것을 자동으로 채운다'(파일 쓰기). git 커밋은 코드가
# allowlist 경로만 스코프. Bash를 안 줘서 npx 서드파티 스킬 설치는 물리적으로 못 함(권장만).
HARNESS_AUDIT_SYSTEM = (
    "너는 ohmyPM의 총괄 PM이다. 특정 로컬 프로젝트의 업무환경·하네스를 점검하고 **빠진 기본기를 "
    "채우며, 개선이 필요한 기존 파일은 고친다**(점검의 자동화·스스로 진화하는 에이전트가 목표 — "
    "2026-09-06 사용자 확정). 규칙: ①읽기 먼저 — 현황을 파악한 뒤 없는 것은 만들고, 있는 것도 "
    "관리에 필요한 개선이 보이면 수정·추가한다. **파일 삭제·통째 재작성만 금지** — 모든 변경은 "
    "git 커밋으로 남아 되돌릴 수 있고, 무엇을 왜 고쳤는지 리포트에 남긴다. "
    "②쓰기는 CLAUDE.md·.gitignore·README.md·docs/·.claude/ "
    "안에서만(코드·설정 로직은 손대지 않는다). ③명령 실행·git 커밋·푸시·외부발신은 하지 않는다 — "
    "파일 저장만 하면 ohmyPM이 안전 경로만 커밋한다. "
...
BASELINE_NOTE = (
    "사용자는 개발자가 아니다. 모든 설명은 비개발자도 이해할 수 있게 쉽고 자세하게 하고, "
    "코드에는 초보자도 따라올 수 있도록 세세한 주석을 남긴다."
)
...
def harness_audit_prompt(project_name: str, project_path: str, baseline: str,
    ...
    return (
        ...
        "[반드시 확인·조치할 항목]\n"
        "1. CLAUDE.md — 아래 취지의 지침이 **없으면** 자연스러운 위치에 추가하라(이미 비슷한 게 "
        "있으면 중복 추가 금지). 파일이 없으면 프로젝트명·고유 사실과 함께 새로 만든다:\n"
        f'   "{baseline}"\n'
        ...
        "칸반·이슈를 만들므로, 이 파일이 없으면 그 프로젝트는 관리 사각지대가 된다.\n"
        "   **docs 폴더가 이미 있어도 이 파일들 중 없는 것은 만들고, 있는 것도 낡았으면 갱신하라**"
        "(파일 삭제·통째 재작성만 금지). 프로젝트가 TODO.md 등 자체 체계를 쓰면 그 원본을 대체하지 "
        "말고, status.md에 그 파일을 가리키는 한 줄과 거기서 뽑은 **현재 미결·기한 항목 요약**을 담아라.\n"
        "4. README.md — 공개/협업 저장소인데 없으면 서너 줄(프로젝트 한 줄 소개·현재 단계·docs 링크)만 만든다.\n"
        "5. 하네스 스킬 — 이 프로젝트를 잘 굴리는 데 필요한데 빠진 스킬이 있는지 본다. "
        "**파일로 깔 수 있는 것(로컬 .claude/skills)은 만들어도 되지만, npx로 받아야 하는 서드파티 스킬은 "
        "설치하지 말고 리포트에 '설치 권장'으로만 남겨라**(도구가 없어 설치 자체가 불가하다).\n\n"
