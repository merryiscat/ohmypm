...
HARNESS_AUDIT_SYSTEM = (
    ...
    "직접 채운다**. 규칙: ①읽기 먼저 — 이미 있는 건 건드리지 말고, **명백히 빠진 안전한 것만 새로 "
    "만든다**(기존 파일 덮어쓰기·삭제 금지). ②쓰기는 CLAUDE.md·.gitignore·README.md·docs/·.claude/ "
...
def harness_audit_prompt(project_name: str, project_path: str, baseline: str) -> str:
    ...
    return (
        ...
        "   **docs 폴더가 이미 있어도 이 파일들 중 없는 것은 만들어라**(기존 파일 수정·삭제는 금지, "
        "새 파일 추가만). 프로젝트가 TODO.md 등 자체 체계를 쓰면 그 원본은 그대로 두고, status.md에 "
        "그 파일을 가리키는 한 줄과 거기서 뽑은 **현재 미결·기한 항목 요약**을 담아라.\n"
