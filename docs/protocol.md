<!-- kickoff-workspaces v0.3.2 — 원본은 ohmypm 플러그인. 여기서 고치지 말고 플러그인을 고친 뒤 "구조 업데이트하자" -->
...
| **M 표준** | 여러 파일 · 판단이 조금 필요 · 되돌릴 수 있음 | pl 스펙 한 장(기준 ≤ 5, 30줄) → pl2 검토는 **차단 항목만, 10줄 이내**(차단 없으면 "없음" 한 줄, v2 생략) → 게이트 없음(roles.md가 켜지 않는 한) → 워커 → pl2 검증은 기준별 한 줄(재조회는 차단 기준만) | 10분 안팎 |
...
## 토큰 규율 — pl(Codex)은 쓰는 순간에만 쓴다

pl 자리의 모델은 종량·소액 쿼터다(2026-09-17 사용자: "함부로 토큰 날리면 치명적"). 그래서 **pl은 두 번만 켠다**:
스펙 v1 작성, 검토 반영 v2 작성. 그 밖의 모든 것 — 검토, 게이트, 워커 배정, 대기, 검증, 머지 — 은 정액제인 pl2(Claude)가 한다.
...
pl2도 워커 일을 대신하지 않는다 — 검토·배정·판정만. 구현·문서 작성은 등급 모델 워커가 새 워크트리에서.
...
pl:  스펙 v1 (docs/tasks/T-NNN-<slug>.md)                          ← Codex 켜는 순간 1
  ...
  → pl2: 차단 지적이 기각됐나?  예 → 게이트(사용자 결정)   아니오 → 게이트(스펙 승인)
  → pl2: 등급에 맞는 워커 배정 (새 워크트리), check --wait
  → 워커: 구현 + worker_done (완료 기준 항목별 통과/실패)
  → pl2: 스펙 기준으로 검증 — 통과/실패만, 고치지 않는다
  → main: 머지·푸시
```
...
**차단 지적을 기각했다면 토론하지 않는다** — pl은 "차단 기각 있음"이라고 보고하고 멈추며, pl2가 게이트로 올린다:
```
...
스펙 승인은 사용자 몫이다 — **L 차선만**. M은 roles.md에 `게이트: M` 이 있을 때만 묻는다. 승인 뒤에만 워커를 띄운다. 이 절은 pl2가 실행한다 — pl은 배정·대기 턴을 쓰지 않는다.
...
워커 브랜치를 읽고 완료 기준마다 통과/실패를 판정해 스펙의 "검증" 절에 적는다(pl2가 스펙 파일을 쓰는 유일한 경우). **M 차선은 기준별 한 줄, 재조회·재실행은 차단 기준에만** — 전부 재검증하면 검증이 워커만큼 걸린다(실측 5분).
...
| pl2 (Claude) | 태스크가 **main에 머지되고 워커 워크트리를 지운 뒤** | ① worker_done 받은 직후, 검증(큰 산출물 읽기) 전 ② 반복 업무를 같은 세션에서 여러 회 돌릴 때 회차 사이 ③ 대기 중 질문 왕복이 길어졌을 때 | 검토·배정·대기·판정 중 clear(run·task·dispatch id·핸들을 잃는다) |
