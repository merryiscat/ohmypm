...
| **M 표준** | 여러 파일 · 판단 조금 · 되돌릴 수 있음 | 2절(짧은 스펙) → 3절(요청 대조 + 차단만) → 4절(10초 게이트 → 배정) → 5절 |
| **L 중대** | 운영 · 되돌리기 어려움 · 구조 변경 · 미지 | 2~5절 전부, 게이트 있음 |

**기본은 S다** — 되돌리기 어렵거나 운영에 닿을 때만 올린다(protocol 0절). 사용자가 "가볍게"·"제대로"라고 하면 그게 이긴다. 판정 결과를 한 줄로 말하고 시작한다("M으로 갑니다 — 파일 셋, 되돌릴 수 있음").
...
한 줄. **요청 원문을 그대로** 싣고, 무엇을 만들라는지만 덧붙인다. **배경·해석·유사 태스크 언급을 덧붙이지 않는다** — 2026-09-18 T-003에서 main이 "T-002 테스트셋을 재사용하라"를 배경으로 붙였고 T-002의 A/B 평가 설계가 통째로 따라왔다. 이전 산출물을 꼭 가리켜야 하면 **따를 것과 따르지 말 것을 같이** 적는다:
```
...
orca terminal send --terminal <pl2> --enter --wait-submit 10 --json --text \
 "pl이 docs/tasks/T-NNN-<slug>.md v1을 완성했다(차선 <M|L>). protocol 2절대로 docs/reviews/T-NNN.review-v1.md를 한 번만 쓰고 결론을 한 줄로 보고하라. **0번 요청 대조를 먼저 적어라** — 스펙의 '요청 원문'과 완료 기준이 같은 것을 말하나. 원문에 없는 산출물·열·행·판정이 기준에 있으면 차단이다. M이면 그 뒤 차단 항목만 10줄 이내, 차단 없으면 '대조: 일치 / 지적 없음' 한 줄. 다른 파일은 건드리지 않는다."
```
...
스펙·검토서를 pl 브랜치에 커밋 → main에 ff 머지 → **푸시**(원격이 있으면) → **게이트**. L은 스펙 목표·범위 밖·산출물을 두 문장으로 묻는다. **M은 10초 게이트** — 용도 한 줄과 완료 기준 목록만 그대로 보여 주고 "이대로 갑니다" 한 줄. 사용자가 착수 전에 스펙을 보는 유일한 지점이라 생략하지 않는다. 그 다음 **4a 배정·대기**:
```
