...
직접 한다: 문서 한 줄·오타, 머지·푸시·브랜치 정리, 질문에 답하기, 이미 승인된 스펙의 워커 결과 머지.
넘긴다: 그 밖의 전부 — 코드 변경, 분석·조사, 문서 새로 쓰기, 반복 운영 업무의 **첫 회**.
**반복 운영 업무**(예: 금일 로그 검토)는 첫 회에만 스펙을 만들고, 이후는 스펙 파일을 재사용해 **pl 없이 pl2가 워커만 재배정**한다(4절 — pl 턴 0).
...
orca terminal send --terminal <pl2> --enter --wait-submit 10 --json --text \
 "pl이 docs/tasks/T-NNN-<slug>.md v1을 완성했다. protocol 2절대로 docs/reviews/T-NNN.review-v1.md를 한 번만 쓰고 결론을 한 줄로 보고하라. 다른 파일은 건드리지 않는다."
```
검토서의 `결론`이 **차단 없음**이면 5절로. 차단 있음이면 pl에 v2 지시(Codex 턴 2) — 사용자 결정이 필요한 지적이 섞여 있으면 **v2 지시 전에** 사용자에게 물어 결정을 지시문에 싣는다(턴 절약):
```
...
- main이 코드·분석·문서 작성을 대신하기(가장 흔한 실패 — "금방 하니까"가 구조를 죽인다)
