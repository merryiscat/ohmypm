...
[main]  내 원본 폴더            ─ 합치고(머지) 올리기(푸시)만 한다. 일은 안 한다.
   ...
   │     ├─ pl2  (Claude 페이블) 스펙을 한 번 검토한다. 그것뿐
   │     └─ pl3  (Claude 오퍼스) 일을 나눠 주고, 기다리고, 결과를 채점한다
   │
...
5. 내가 승인한다.
6. pl3가 난이도에 맞는 워커를 새 워크스페이스에 띄우고 기다린다.
7. 워커가 끝내면 pl3가 스펙의 완료 기준으로 통과/실패를 매긴다.
8. main이 합치고 올린다.
...
| Claude Code (Max 플랜) | pl2·pl3·워커·main 자리 | `claude --version` |
...
main이 먼저 **차선**을 정해 한 줄로 말합니다.

| 차선 | 어떤 일 | 어떻게 | 시간 |
|---|---|---|---|
| S 즉시 | 파일 한두 개, 요청이 분명, 되돌리기 쉬움 | main이 그 자리에서 하고 커밋 | 3~5분 |
| M 표준 | 파일 여럿, 판단 조금 | pl 짧은 스펙 → pl2 차단만 검토 → 워커 → 짧은 검증. 승인 질문 없음 | 10분 안팎 |
| L 중대 | 운영·되돌리기 어려움·구조 변경 | 위 1절의 바퀴 전부. 승인 질문 한 번 | 20~30분 |

"가볍게 해 줘", "제대로 해 줘"라고 덧붙이면 그 말이 차선을 정합니다.

- pl 터미널에 직접 말을 걸지 마세요. 아스트라가 그 자리에서 조사까지 하며 턴을 씁니다. 요청은 main에.
- 매일 반복하는 일(로그 검토 같은 것)은 첫 회만 스펙을 만들고, 다음부터는 pl 없이 pl3가 워커만 다시 띄웁니다.
...
**Q. 너무 느려요.**
차선을 확인하세요. 작은 일이 L로 가면 20분이 넘습니다. "가볍게"라고 붙이면 S로 갑니다. 매일 반복하는 일은 첫 회 뒤로는 스펙을 재사용해 훨씬 빠릅니다.
...
**Q. pl·pl2·pl3 세션은 언제 비우나요? `/clear`와 `/compact` 중 뭘 쓰나요?**
작업 하나가 **끝나면** `/clear`(pl은 스펙 v2를 낸 뒤, pl2는 검토서를 낸 뒤, pl3는 그 작업이 합쳐진 뒤). 기억은 스펙·검토서 파일에 있으니 잃는 게 없습니다. 작업 **도중**에 pl2가 무거워지면 `/compact`(워커 완료 보고를 받은 직후 검증 전, 반복 업무 회차 사이). pl은 compact를 쓰지 않습니다(그것도 턴 비용). 절차가 알맞은 때에 자동으로 보내고, 규칙 파일을 고쳤으면 둘 다 바로 `/clear`합니다.
