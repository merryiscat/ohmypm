...
**Q. pl·pl2 세션은 언제 비우나요? `/clear`와 `/compact` 중 뭘 쓰나요?**
작업 하나가 **끝나면** `/clear`(pl은 스펙 v2를 낸 뒤, pl2는 그 작업이 합쳐진 뒤). 기억은 스펙·검토서 파일에 있으니 잃는 게 없습니다. 작업 **도중**에 pl2가 무거워지면 `/compact`(워커 완료 보고를 받은 직후 검증 전, 반복 업무 회차 사이). pl은 compact를 쓰지 않습니다(그것도 턴 비용). 절차가 알맞은 때에 자동으로 보내고, 규칙 파일을 고쳤으면 둘 다 바로 `/clear`합니다.

**Q. 일을 하나 시켜 놓고 또 시켜도 되나요? main을 하나 더 열어야 하나요?**
같은 main에 이어서 말하면 됩니다. main은 기다림을 뒤에서 돌리고 다음 요청을 받습니다. 스펙 쓰기(pl)는 한 번에 하나라 순서대로 들어가고, 실제 작업(워커)은 동시에 돕니다. main을 둘 열면 같은 폴더에 합치는 손이 둘이 돼 충돌하니 열지 마세요.
